...
function RefreshCityBanner(cityBanner, iActiveTeam, iActivePlayer)
	...
	if city ~= nil then
		...
		--local israilroadconnected = city:IsIndustrialRouteToCapital();
		...
		if (isActiveTeamCity) then
			if (not city:IsCapital() and player:IsCapitalConnectedToCity(city) and not city:IsBlockaded()) then
				...
				--if(city:IsIndustrialRouteToCapital()) then
				 	--controls.IndustrialConnectedIcon:SetHide(false);--TODO mettere codice per far vedere un icona treno
				--end
				controls.ConnectedIcon:SetToolTipString(Locale.ConvertTextKey("TXT_KEY_CITY_CONNECTED"));
			else
				controls.IndustrialConnectedIcon:SetHide(true);--TODO mettere codice per far vedere un icona treno
