...
local wonderNatHeadingOpen = true;
...
function OnWonderNatHeaderSelected()
	wonderNatHeadingOpen = not wonderNatHeadingOpen;
	UpdateWindow( GetCurrentCity() );
end
...
function UpdateWindow( city )
	...
	--patch gobbi 0.2
	if wonderNatHeadingOpen then
		local localizedLabel = "[ICON_MINUS] "..Locale.ConvertTextKey( "TXT_KEY_POP_WONDERS_NAT" );
		Controls.WondersNatButtonLabel:SetText(localizedLabel);
		Controls.WonderNatButtonStack:SetHide( false );
	else
		local localizedLabel = "[ICON_PLUS] "..Locale.ConvertTextKey( "TXT_KEY_POP_WONDERS_NAT" );
		Controls.WondersNatButtonLabel:SetText(localizedLabel);
...
function AddProductionButton( id, description, orderType, turnsLeft, column, isDisabled, ePurchaseYield )	
	...
	elseif column == 2 or column == 3 or  column == 4 then -- we are a building, wonder, project, or process
		if column == 2 then
			controlTable = g_BuildingInstanceManager:GetInstance();
		elseif column == 3 then
			controlTable = g_WonderInstanceManager:GetInstance();
		elseif column == 4 then
			controlTable = g_WonderNatInstanceManager:GetInstance();
		end
