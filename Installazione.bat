...
@REM     echo.
@REM     echo === Backup dei file esistenti ===

@REM     :: Esegui backup solo dei file che esistono già nella destinazione
@REM     if not exist "%BACKUP%" (
@REM     mkdir "%BACKUP%"
@REM     robocopy "%DEST%" "%BACKUP%" CivilizationV.exe *.xml *.lua *.Civ5Pkg /E /XC /XD UPDATE backup /XN /XO /NFL /NDL /NJH /NJS
@REM     echo Backup salvato in %BACKUP%
@REM     echo.
